...
| `CAP-SUP-001` | Staffing supply — opening HC, attrition, hiring, training, handoff | Strong | Long-range planners/recruiting partners; close monthly supply gaps | `staffingModel.js`, grouped sticky native supply worksheet with explicit opening/ending gaps, `PLAN-008`–`PLAN-010`, date/count/handoff and Chromium geometry tests | No time-weighted in-month supply, skill mix, recruiting capacity constraints, cost, or probabilistic yield | Requirements, calendars | Retain; keep opening and ending position evidence distinct | High | 5/4/4/4/4/4 | 20 | 2026-07-20 |
| `CAP-SCHED-001` | Scheduling — employee shift and schedule optimization | Explicitly out of product scope | No supported persona or product decision; WFM Toolkit stops at staffing requirements and planning outputs | Sponsor direction `INT-005`; no employee, rule, shift, activity, or optimizer modules | Deliberately absent | None; requirement export/integration may be considered only under new sponsor direction | Do not build; prevent roadmap drift into scheduling | High | 0/0/0/0/0/0 | 0 | 2026-07-19 |
| `CAP-SCEN-001` | Scenario planning — versioned plans, updates, and comparison | Strong saved-plan workflow | Planning leads; preserve Budget, create Updates, select current plan, and compare saved operating scenarios | Plan lifecycle, immutable snapshots, required Update reasons, monthly demand/capacity driver attribution, explicit opening/ending gap comparison and complete CSV, `PLAN-013`, `ACT-004`–`ACT-006`, focused and Chromium tests | No explicit what-if branching or parameter-sensitivity generator; legacy Updates can lack rationale | Forecast versions, plan lineage | Retain; add branching only after a concrete decision workflow is established | High | 5/4/4/4/4/4 | 20 | 2026-07-20 |
| `CAP-INTRA-001` | Intraday management — same-day actuals, reforecast, and recovery actioning | Explicitly out of product scope | No supported persona or product decision; interval Erlang remains a staffing-requirement method, not same-day operations management | Sponsor direction `INT-005`; no real-time ingestion, adherence, backlog, reforecast, or recovery-action workflow | Deliberately absent | None | Do not build; keep interval calculations bounded to capacity planning | High | 0/0/0/0/0/0 | 0 | 2026-07-19 |
| `CAP-LRP-001` | Long-range planning — annual demand, requirement, staffing, and updates | Strong | Strategic planners; build, maintain, and compare monthly operating plans | `MonthlyPlanBuilder.vue`, driver-explained monthly plan comparison, traceable Update creation, planner tabs/modules, annual handoff, finalization gates, extensive tests | No explicit what-if branching, parameter sensitivity, or cost view | Forecast, requirement, supply | Retain as core; validate the next scenario decision before adding inputs | High | 5/4/4/4/4/4 | 20 | 2026-07-19 |
| `CAP-REP-001` | Reporting — call-center and staffing-group annual rollups and variance | Strong | Planners/WFM leads; review selected-year demand, requirement, supply, actuals, staffing risk, and saved-plan deltas inside one operating scope | `PlanningCenterView.vue`, sticky contained monthly reconciliation, annual rollup, explicitly named opening-gap reports and opening/ending scenario CSV, `PLAN-012`–`PLAN-013`; `INT-001` retired duplicate cross-center home reporting | Deeper contributor export remains limited; shared pools are unresolved | Reliable plan selection and actuals | Retain inside call-center workflows; improve reconciliation only with decision evidence | High | 5/4/4/4/4/4 | 20 | 2026-07-20 |
| `CAP-REP-002` | Reporting — empty, incomplete, stale, mixed-scope, and error integrity | Strong in retained reporting | WFM leads; decide whether scoped report results are authoritative and what to fix | `ACT-002`, `ACT-003`, and `PLAN-012` now reconcile daily actuals to the saved plan's exact open-date calendar, disclose loaded/expected/missing evidence, withhold partial-month variance and requirements, validate saved Intraday Erlang signatures, and name correction paths; `INT-001` removed the ambiguous cross-center surface | Contributor exports and error-state consistency remain uneven in unreviewed surfaces | All reporting inputs | Retain the authoritative actuals and call-center contracts; improve other reports when encountered | High | 5/4/4/4/5/4 | 17 | 2026-07-20 |
...
| `CAP-UX-001` | Desktop operational UX — dense, keyboard/mouse planning workflows | Adequate-to-strong | All planner personas; scan, edit, compare, and act efficiently on desktop | Shared wrappers, native tables, split center workspace, visible annual-plan readiness, driver- and gap-basis-explained saved-plan comparison, sticky keyboard-scrollable call-center and supply worksheets, focus-safe dialogs, standards/Chromium tests | Keyboard shortcuts, persistent context in remaining dense worksheets, focus restoration in unreviewed workflows, and zoom review are uneven | Shared UI system and workflow-specific testing | Improve within roadmap slices; decline phone-specific work | Medium | 5/3/4/3/4/3 | 32 | 2026-07-20 |

## Change record

### 2026-07-19 — scheduling and intraday management removed from product scope

- Applied sponsor direction `INT-005`: WFM Toolkit will not perform employee shift scheduling or same-day intraday management.
- `CAP-SCHED-001` moved from strategically missing (Need 100) to explicitly out of scope (Need 0). Its absence is no longer an improvement gap.
- `CAP-INTRA-001` moved from strategically missing (Need 80) to explicitly out of scope (Need 0). Interval Erlang staffing calculations remain supported because they produce planning requirements; real-time monitoring, adherence, reforecasting, and recovery actioning do not.
- Future assessments and roadmap reviews must not revive either capability without new explicit sponsor direction.
...
### 2026-07-19 — visible annual-plan readiness

- `CAP-UX-001` remains `5/3/4/3/4/3` (Need 32). The annual plan's persistent workflow rail now exposes every existing section status in text, so a planner can scan applied demand, inherited defaults, requirement coverage, opening-position readiness, and actuals coverage without opening six sections serially.
- `CAP-LRP-001` and `CAP-GOV-001` remain unchanged. The slice makes the existing finalization contract visible but does not change any completeness rule, review state, calculation, or saved payload.
- Removed the unreachable speculative financial-budget panel and its unused upcoming-state navigation dialect. `CAP-FIN-001` remains Explore because rate, currency, burden, overtime, vendor, and Finance reconciliation semantics are still undefined.

### 2026-07-19 — persistent staffing-supply decision context

- `CAP-UX-001` remains `5/3/4/3/4/3` (Need 32). The monthly staffing-supply worksheet now has one named keyboard scroll region, semantic roll-forward column groups, sticky two-tier headings, and a sticky Month column across supported desktop widths, zoom equivalents, and reduced desktop windows.
- `CAP-SUP-001` remains `5/4/4/4/4/4` (Need 20). Planners can reconcile requirement, opening supply, pipeline and loss, ending supply, and gap without changing any roster, graduation, attrition, headcount, recommendation, or handoff calculation.
- Scores remain calibrated because Agent Availability, Demand Model, Actuals, and other dense worksheets still have uneven persistent context and product-wide keyboard shortcuts remain limited.
- Replaced the staffing table's one-off assumption shell with shared `AppTableShell`; added month-specific attrition input names and removed no data, route, state, formula, compatibility layer, or phone behavior.

### 2026-07-19 — monthly scenario driver attribution

- `CAP-SCEN-001` remains `5/4/4/4/4/4` (Need 20). Saved Budget-versus-Update comparison now attributes each material month to exact changes in contacts, AHT, open days, paid hours per day, presence, occupancy, adherence, and peak-day uplift before showing average and peak-day requirement, supply, and gap outcomes.
- `CAP-LRP-001` remains `5/4/4/4/4/4` (Need 20). The complete 12-month CSV now carries baseline, candidate, and delta evidence for every driver and outcome while same-method and saved-snapshot rules remain unchanged.
- `CAP-UX-001` remains `5/3/4/3/4/3` (Need 32). The dialog is scanable and contained at supported desktop and zoom-equivalent widths, but product-wide shortcuts, remaining worksheet context, and unreviewed focus/zoom paths remain uneven.
- Removed four annual-average capacity rows and the comparison-only averaging helper because offsetting monthly assumption changes could falsely appear unchanged. Explicit what-if branching and sensitivity generation remain absent, so scores stay calibrated.

### 2026-07-20 — actuals open-date coverage authority

- `CAP-REP-002` improves from `5/4/4/4/4/4` (Need 20) to `5/4/4/4/5/4` (Need 17). Monthly contacts, AHT, workload, requirement, and staffing-gap comparisons are now authoritative only when daily actuals cover every expected open date from the saved plan calendar; exact loaded, expected, and missing-date evidence remains visible when they do not.
- `CAP-GOV-001` remains `4/3/4/3/3/4` (Need 26). The saved calendar is now an explicit authority boundary for actuals, but product-wide identity, approvals, ownership, and event history are still absent.
- `CAP-UX-001` remains `5/3/4/3/4/3` (Need 32). The Actuals worksheet now supplies grouped sticky headings, a sticky Month column, contained keyboard scrolling, non-color coverage states, and clear corrective guidance at supported desktop widths; other dense workflows remain uneven.
- Consolidated duplicate plan-update coverage logic into the shared actuals model and replaced the Actuals worksheet's one-off overflow shell with `AppTableShell`. Raw observations remain visible; only unsupported comparison and staffing conclusions are withheld. No stored-data migration, route, new formula, phone layout, or speculative scheduling/intraday surface was added.

### 2026-07-20 — explicit opening and ending staffing gaps

- `CAP-SUP-001`, `CAP-SCEN-001`, and `CAP-REP-001` remain `5/4/4/4/4/4` (Need 20). The staffing worksheet and saved-plan comparison now distinguish the opening position from the ending position after planned movements without changing the retained report basis.
- `CAP-UX-001` remains `5/3/4/3/4/3` (Need 32). Dense worksheet, plan-library, call-center, dialog, and CSV labels now name the gap basis, but broader keyboard, focus, and remaining worksheet-context gaps still prevent a product-wide score increase.
- Opening gap remains `opening frontline HC - required HC`; ending gap remains `ending frontline HC - required HC`. Negative values indicate shortage. The existing `gapToRequirement` and `averageGapToRequirement` fields remain opening-gap compatibility aliases so saved plans and backups require no migration.
- Removed generic staffing-gap presentation and CSV headings exposed by this slice; no scheduling, same-day intraday management, employee data, stored state, requirement formula, recommendation rule, or route was added.
