...
| `CAP-REQ-002` | Staffing requirements — interval Erlang C/A | Strong calculation, adequate workflow | Capacity planners; size interval staff to service, occupancy, and abandonment targets | Python reference vectors, FastAPI Erlang APIs, shared saved-result signature validation, call-center rollup withholding, calculator and planner tests | No multi-skill/network routing; backend round trip and unretained actual Erlang results add workflow friction | Interval forecast/profile and service goals | Retain; persist matching actual requirement evidence before expanding reporting | High | 5/4/4/3/4/4 | 24 | 2026-07-19 |
...
| `CAP-REP-002` | Reporting — empty, incomplete, stale, mixed-scope, and error integrity | Adequate-to-strong, improving | WFM leads; decide whether scoped report results are authoritative and what to fix | `PLAN-012` call-center rollup validates saved Intraday Erlang signatures, withholds stale/missing and mixed actual-requirement scope, and names correction paths; `INT-001` removed the ambiguous cross-center surface | Error-state consistency and contributor exports remain uneven outside the retained report | All reporting inputs | Retain the authoritative call-center contract; improve other reports when encountered | High | 5/3/4/4/4/4 | 25 | 2026-07-19 |
...
| `CAP-ADMIN-001` | Administration — identity, roles, permissions, tenancy | Strategically missing | Admins/WFM leaders; control access and separation of duties | No application identity or authorization workflow; local browser storage is device-scoped; `INT-003` requires plan edit locks | Entire capability absent; lock owner and authorized recovery depend on the future user system | `CAP-DATA-002`, product tenancy/user decision, and exclusive-lock lifecycle from `INT-003` | Later with shared persistence; do not add cosmetic role UI first | High | 3/0/0/0/0/0 | 60 | 2026-07-19 |
