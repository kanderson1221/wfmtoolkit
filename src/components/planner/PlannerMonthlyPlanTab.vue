...
const selectedMonth = computed(
  () => props.monthlyRecords[selectedMonthIndex.value] ?? props.monthlyRecords[0] ?? { planWarnings: [] }
)
  ...
  <section class="results-panel monthly-tab-panel">
    ...
    <div class="results-metrics monthly-summary-grid">
      ...
      <article class="metric-card">
        ...
        <p class="metric-meta">{{ props.planSummary.busiestMonth.fullLabel }} is the busiest workload month</p>
      </article>
      <article class="metric-card">
          ...
          <div class="monthly-bar-stack">
            <div class="monthly-bar-track">
              <div
                ...
                :style="{
                  height: `${Math.max((record.requiredStaffHours / props.monthlyChartMax) * 100, record.requiredStaffHours > 0 ? 6 : 0)}%`
                }"
              ></div>
            </div>
          </div>
